...
public class KafkaService {
	...
	private static final HiveService consumerService = HiveService.getInstance();
	...
	public static JavaInputDStream<ConsumerRecord<String, String>> subscribe(String topic) {
        ...
        // consume data from Kafka and save to Hive
 		stream.foreachRDD(rdd -> rdd.foreach(record -> {
             if (record != null) {
                 //logger.info("Partition:" + record.partition() + ",Offset:" + record.offset());
                 
                 consumerService.insertWeathers(record.value());
             }
         }));
